...
extern "C"
    ...
    /*!
     * \brief Depthwise Conv2D function for mock-accelerator examples. Limited to same-padded Conv2D with
     * stride (1,1) and datatype float. 
     ...
     *
     */
int vanilla_extended_depthconv2dnchw(float* ifmap, float* weights, float* result, int ic, int oc, int iw, int ih, int kh, int kw) {
  ...
  float* pad_temp = (float*)malloc(
      (((oc * padded_iw * padded_ih) + (padded_ih * padded_iw)) + padded_iw) * sizeof(float));
      ...
      for (int i3 = 0; i3 < padded_iw; ++i3) { 
        ((float*)pad_temp)[(((i1 * padded_iw * padded_ih) + (i2 * padded_iw)) + i3)] =
            (((((kh_low <= i2) && (i2 < kh_high)) && (kw_low <= i3)) && (i3 < kw_high))
                 ? ifmap[((((i1 * iw * ih) + (i2 * iw)) + i3) - kw_high)]
                 : 0.000000e+00f);
      }
    }
  }
